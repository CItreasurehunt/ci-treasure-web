-- I-101 -- add a check for upcoming published events with no image_url. Not a rendering bug like
-- the malformed-links/legacy-price checks -- these events just render with no flyer/photo, which
-- is a content-completeness gap rather than a defect, but it's the exact gap manually cleared to
-- zero on 2026-07-10 (25 events backfilled via scripts/enrichment/image_tracker.csv) and worth
-- catching automatically going forward rather than rediscovering by manual audit again.
